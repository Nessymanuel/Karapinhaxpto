    ...
    try {
      ...
      if (response.data.success) {
        ...
        const userResponse = await api.get(
          `https://localhost:7104/api/User/details/${formData.email}`,
          {
            headers: {
              "Content-Type": "application/json"
            }
        ...
        // Armazenar dados do usuário no contexto de autenticação
        login(userData);
        ...
        setAlertMessage(
          "Credenciais inválidas. Crie uma conta para acessar."
        );
      }
    } catch (error) {
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (error.response) {
          ...
          setAlertMessage(
            "Erro ao conectar ao servidor. Verifique sua conexão ou tente novamente mais tarde."
          );
        }
      } else {
        setAlertMessage(
      ...
      <div className="bg-white p-8 rounded-md shadow-md">
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            <label
              htmlFor="email"
              className="block text-gray-700 font-regular mb-2"
            >
              Email:
            </label>
            <input
        ...
        <div className="flex gap-1">
          ...
          <a className="flex justify-center text-amber-700" href="SignUp">
            Cadastre-se
          </a>
        </div>
      </div>
